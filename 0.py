...
from scapy.all import arp_mitm, sniff, DNS, srp, Ether, ARP, send, IP, UDP, DNSRR, TCP
...
class Device:
    ...
    def smb_telnet_sniff(self):
        def smb_telnet_pkt_callback(pkt):
            # SMB Traffic (Port 445)
            if pkt.haslayer(IP):
                ip_src = pkt[IP].src
                ip_dst = pkt[IP].dst
                if pkt.haslayer(TCP) and (pkt[TCP].dport == 445 or pkt[TCP].sport == 445):
                    print(f'{Fore.GREEN}SMB Connection Detected: {ip_src} <-> {ip_dst}{Style.RESET_ALL}')
            ...
            if pkt.haslayer(IP):
                ...
                if pkt.haslayer(TCP) and (pkt[TCP].dport == 23 or pkt[TCP].sport == 23):
                    print(f'{Fore.CYAN}Telnet Connection Detected: {ip_src} <-> {ip_dst}{Style.RESET_ALL}')

        sniff(iface=self.iface, prn=smb_telnet_pkt_callback, filter="tcp port 445 or tcp port 23", store=0)
